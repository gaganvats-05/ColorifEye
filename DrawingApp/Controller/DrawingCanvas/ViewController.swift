...
extension ViewController: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func image()  {
        ...
        let image = Image<RGBA<UInt8>>(named: "brush")!
        let image2:UIImage? = #imageLiteral(resourceName: "brush-1")

        let height = Int((image2?.size.height)!)
        print(height)
        let width = Int((image2?.size.width)!)
        print(width)
        for pixel in image {
              print(pixel.red)
              print(pixel.green)
